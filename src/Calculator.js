...
function Calculator(props) {
    ...
    const forwardClick = () => {
      if(step==1){
        ...
        taxIncomeLabel.current.innerHTML = textInput.current.value - textInput2.current.value - 12400;
        taxBillLabel.current.innerHTML = 0.1 * (textInput.current.value - textInput2.current.value - 12400);
      } else if (step==3) {
        creditsLabel.current.innerHTML = textInput3.current.value;
      } else if (step == 4) {
        refundLabel.current.innerHTML = textInput3.current.value
        ...
        <Row type="flex" justify="center" align="middle">
          ...
          <Row type="flex" justify="center" align="middle">
            ...
            <Col>
              ...
              <button className="calc-button" onClick={() => forwardClick()} style={{display: step>=5 ? 'none' : ''}}>Keep Going</button>
            </Col>
          </Row>
          </Col>
          <Col>
